...
import Items from '../AppItem'
import TabItems from '../TabItem'

class AppStore extends Component {
  state = {
    searchInput: '',
    appData: tabsList[0].tabId,
  }

  updateDataFilter = () => {
    let {appData} = this.state
    let data = appsList.filter(each => {
      return each.category === appData
    })
    return data
  }
  ...
  updateState = props => {
    this.setState({appData: props})
  }

  render() {
    let {searchInput, appData} = this.state
    console.log(searchInput)
    let filterData = this.updateDataFilter()

    let searchFilter = filterData.filter(each => {
    ...
    return (
      <div className="container">
        ...
        <div className="searchbar">
          <div className="innerbarsearch">
            <input type="search" onChange={this.click} />
            <img
        ...
        <div className="ImageCards">
          <ul className="imgCards">
            {searchFilter.map(each => {
              return <Items cardObjeact={each} key={each.appId} />
            })}
          </ul>
        </div>
      </div>
